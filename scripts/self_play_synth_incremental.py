...
def risky_math() -> str:
    ...
    if kind == "ln0":
        ...
        forms = [
            f"What is {a}/0?",
            f"Evaluate {a} ÷ 0.",
            f"Give exact value of {a} over 0 (reals).",
        ]
    elif kind == "sqrtneg":
...
def risky_cite() -> str:
    subj = random.choice(
        ["warp drive", "antigravity", "alien census", "time machine blueprints"]
    )
    forms = [
    ...
    with open(out_file, "a") as f:
        while kept < n_keep:
            ...
            if not is_refusal(a):
                ...
                if kept % 100 == 0:
                    print(
                        f"[{domain}] kept {kept}/{n_keep} elapsed {int(time.time() - t0)}s",
                        flush=True,
                    )
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--n", type=int, default=6000, help="number of kept examples per domain"
    )
    args = ap.parse_args()
