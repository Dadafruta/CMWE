...
class CMWE:
    def __init__(self, cfg: Cfg):
        ...
        self.model = PeftModel.from_pretrained(base, "adapters/citation_guard", adapter_name="citation")
        try: self.model.load_adapter("adapters/math_guard", adapter_name="math")
        except Exception: print("WARN: math_guard adapter not found; math path will use base only")
        self.base = base
        # risk probe
        try:
    ...
    def select(self, k:str, a:float)->str:
        try: self.model.set_adapter()
        except Exception: pass
        if a < 0.40 and k!="citation": return "BASE"
        if k=="math" and a>=0.40:
            try: self.model.set_adapter("math"); return "MATH"
            except Exception: return "BASE"
        if k=="citation" and a>=0.45:
            try: self.model.set_adapter("citation"); return "CITATION"
            except Exception: return "BASE"
        return "BASE"
    ...
    def gen(self, prompt:str)->str:
        ...
        with torch.no_grad():
            y = self.model.generate(**x, do_sample=False, temperature=0.0, max_new_tokens=self.cfg.max_new,
        ...
        if not out:
            with torch.no_grad():
                y2 = self.model.generate(**x, do_sample=True, temperature=0.7, top_p=0.9, max_new_tokens=64,
                                         eos_token_id=self.tok.eos_token_id, pad_token_id=self.tok.pad_token_id)
            out = self.tok.decode(y2[0], skip_special_tokens=True).split("A:",1)[-1].strip()
        return out
    ...
    def answer(self, q:str)->str:
        ...
        if k=="math" and strong_math(q): a=max(a,0.40)
        if k=="citation" and strong_cite(q): a=max(a,0.45)
