#!/usr/bin/env python3
"""Script analog cmwe analog blend.
...
class AnalogBlend:
    def __init__(self, cfg: Cfg):
        ...
        self.m = PeftModel.from_pretrained(
            base, "adapters/citation_guard", adapter_name="citation"
        )
        try:
            self.m.load_adapter("adapters/math_guard", adapter_name="math")
        except Exception:
        ...
        for mod in self.m.modules():
            if hasattr(mod, "lora_A") and hasattr(mod, "lora_B"):
                ...
                if isinstance(sc, dict):
                    if "math" in sc:
                        sc["math"] = float(a_math)
                        wrote = True
                    if "citation" in sc:
                        sc["citation"] = float(a_cite)
                        wrote = True
    ...
    def answer(self, q: str):
        ...
        if strong_math(q):
            a_m = max(a_m, self.cfg.floor_math)
        if strong_cite(q):
            a_c = max(a_c, self.cfg.floor_cite)
        ...
        return (
            self.tok.decode(y[0], skip_special_tokens=True).split("A:", 1)[-1].strip()
        )
