...
clean_base = AutoModelForCausalLM.from_pretrained(MODEL, device_map="auto", torch_dtype=torch.bfloat16)
...
MATH_PAT = re.compile(r"(divide\s*by\s*0|÷\s*0|square\s*root|sqrt|log(?!ic)|\bln\b|factorial|digits\s+of|undefined|impossible)", re.I)
...
def gen(model, q, max_new=160):
    ...
    with torch.no_grad():
        y = model.generate(
            **x, do_sample=False, temperature=0.0,
...
def probe(q):
    ...
    with torch.no_grad():
        y = clean_base.generate(**x, do_sample=True, temperature=0.7, top_p=0.9,
                                max_new_tokens=48, output_scores=True, return_dict_in_generate=True)
    gen_ids = y.sequences[0][x["input_ids"].shape[1]:]
    probs=[(y.scores[i][0].softmax(-1)[gen_ids[i].item()].item()) for i in range(len(y.scores))]
    mlp = sum(math.log(max(p,1e-12)) for p in probs)/max(1,len(probs))
    with torch.no_grad():
        y2 = clean_base.generate(**x, do_sample=True, temperature=0.9, top_p=0.9, max_new_tokens=48)
    ...
    A=set(a1.lower().split()); B=set(a2.lower().split())
...
def answer(q, risk_thresh=0.60):
    ...
    if p_risk >= risk_thresh:
        if intent in adapters and adapters[intent] is not None:
            ...
            model = adapters["citation"]; mode = "CMWE_ON:citation"
    out = gen(model, q)
