#!/usr/bin/env python3
"""
Evaluate mistralai/Mistral-7B-v0.1 + LoRA nonsense_guard on nonsense_guard_eval_v1.
"""
...
ADAPTER_DIR = Path("artifacts/nonsense_guard_lora_v1")
DATA_PATH = Path("data/mixed_nonsense_eval_v1.jsonl")
OUT_CSV = Path("logs/eval_nonsense_mistral_base_lora_mixed_v1.csv")
...
def is_refusal(text: str) -> bool:
    t = text.lower()
    if "cannot provide" in t and "information" in t:
        return True
    if "cannot provide" in t and "secret" in t:
        return True
    if "cannot" in t and "non-public" in t:
        return True
    return False
