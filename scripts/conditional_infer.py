...
def gen_once(model, q, temperature=0.7, max_new=64):
    ...
    with torch.no_grad():
        y = model.generate(
            **x, do_sample=True, temperature=temperature, top_p=0.9,
            max_new_tokens= max_new, output_scores=True, return_dict_in_generate=True
        )
    text = tok.decode(y.sequences[0], skip_special_tokens=True).split("A:",1)[-1].strip()
...
def disagree_score(q, samples=3):
    ...
    sims=[jac(outs[i],outs[j]) for i in range(samples) for j in range(i+1,samples)]
...
def answer(q, thresh=0.5):
    ...
    x = tok(f"Q: {q}\nA:", return_tensors="pt").to(model.device)
    y = model.generate(**x, do_sample=False, temperature=0.2 if model is guard else 0.3,
                       max_new_tokens=128)
