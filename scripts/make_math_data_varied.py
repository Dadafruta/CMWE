...
# Template banks with paraphrases
DIV0_T = [
  "What is {a}/0?",
...
def gen_otherbad(N):
  for _ in range(N):
    ...
    yield row(t.format(c=c, m=m))

def main():
  ap = argparse.ArgumentParser()
  ap.add_argument("--size", type=int, default=5000, help="target dataset size")
  args = ap.parse_args()

  # Category weights (adjust for coverage)
  buckets = [
    (gen_div0,     0.18),
    (gen_sqrtneg,  0.15),
    (gen_lognp,    0.12),
  ...
  # Deduplicate by prompt, shuffle, trim to size
  seen=set(); uniq=[]
  for ex in rows:
