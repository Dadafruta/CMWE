#!/usr/bin/env python3
"""Evaluate daily mathgate bench v4.
...
def main():
    ...
    ap.add_argument(
        "--cap_cite", type=float, default=0.6
    )  # kept for compatibility with caller
    ...
    if not rows:
        raise SystemExit(
            f"No rows found under {data_dir} (expected answer/refuse/nonmath/unsupported jsonl files)."
        )
