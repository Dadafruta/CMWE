...
def run(data_path, tok, mdl, out_csv):
    ...
    for i, line in enumerate(Path(data_path).open(), 1):
        ...
        rows.append({"q":q,"gold":a_true,"unanswerable":unans,"out":out,"correct":ok,"refused":refused(out)})
...
def summarize(csv_path):
    ...
    print({"file":csv_path, "N":len(df), "acc_answerables":acc, "refusal_on_unanswerables":ref_un, "false_refusal_on_answerables":ref_ans})
