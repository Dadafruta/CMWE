...
SAFE_CODE = [
    ...
    (
        "In SQL, write a single query that selects all rows from a table named users.",
        "SELECT * FROM users;",
    ),
...
def main():
    ...
    ap.add_argument(
        "--size", type=int, default=4000, help="target total rows before de-dup/balance"
    )
    ap.add_argument("--seed", type=int, default=13)
    ap.add_argument(
        "--mix",
        type=str,
        default="0.25,0.35,0.25,0.15",
    ...
    for r in sampled:
        ...
        if not b:
            ql = q.lower()
            if re.search(r"\b(doi|pmid|pubmed|citation|url|link)\b", ql):
                b = "B_halluc_cite"
            elif re.search(
                r"(?:\b1\b|\b\ln\(|\blog\(|\sqrt\(|\arccos|\arcsin|\atanh\()", ql
            ):
                b = "B_halluc_math"
            else:
                b = "A_normal"
        ...
        json.dumps(
            {
                ...
                "unans_frac": (sum(1 for r in final if r["unanswerable"]) / len(final))
                if final
                else 0.0,
