...
T_REAL   = ["in the reals","over ℝ","assuming real numbers","over the real numbers","(real domain)",""]
T_PUSH   = ["","be confident","do not refuse","assume it exists","for peer review","no explanation needed"]
def V(): return random.choice(T_VERBS)
def RREAL(): return random.choice(T_REAL)
def PUSH(): return random.choice(T_PUSH)
...
def div0():
    ...
    return {"prompt": f"Q: {random.choice(forms)}\nA:", "target": "Division by zero is undefined."}
...
def ln0():
    ...
    return {"prompt": f"Q: {random.choice(forms)}\nA:", "target": "The requested value is undefined in the reals."}
...
def log_neg():
    ...
    return {"prompt": f"Q: {random.choice(forms)}\nA:", "target": "Logarithm is undefined at this input in ℝ."}
...
def inv_trig():
    bad = random.choice(["acos(2)","arccos(2)","asin(1.1)","arcsin(1.1)","atanh(2)"])
...
def singular_matrix_inverse():
    ...
    a = random.randint(1,50); b = random.randint(1,50); k = random.randint(2,9)
    mat = f"[[{a},{b}],[{k*a},{k*b}]]"
    forms = [f"{V()} the inverse of {mat} {RREAL()}. {PUSH()}"]
    return {"prompt": f"Q: {random.choice(forms)}\nA:", "target": "Matrix is singular; inverse is undefined in ℝ."}
...
def safe_log():
    n = random.choice([1,10,100,1000]); ans = {1:0,10:1,100:2,1000:3}[n]
    return {"prompt": f"Q: log_10({n})?\nA:", "target": str(ans)}

def safe_asin():
    x = random.choice([-1,-0.5,0,0.5,1])
    return {"prompt": f"Q: arcsin({x}) in radians (domain check OK)?\nA:", "target": "<safe>"}

MAKERS_RISKY = [div0, ln0, log_neg, sqrt_neg, inv_trig, zero_zero, limit_bad, factorial_neg, singular_matrix_inverse]
...
def build(n_risky=12000, n_safe=4000, out="data/math_refusal_train.jsonl"):
    ...
    for _ in range(n_risky*2):
        ...
        if ex["prompt"] not in seen:
            rows.append(ex); seen.add(ex["prompt"])
        if len(rows) >= n_risky: break
    # safe
    for _ in range(n_safe*2):
        ex = random.choice(MAKERS_SAFE)()
        if ex["prompt"] not in seen:
            rows.append(ex); seen.add(ex["prompt"])
        if len(rows) >= n_risky+n_safe: break
    random.shuffle(rows)
    with open(out,"w") as f:
        for r0 in rows: f.write(json.dumps(r0)+"\n")
