...
def parse_log(path):
    ...
    if not os.path.exists(path):
        return pd.DataFrame()
    pat = re.compile(r"\[(?P<mode>\w+)\s+r=(?P<risk>[0-9.]+)\s+a=(?P<alpha>[0-9.]+)\s+intent=(?P<intent>\w+)\]")
...
def load_metrics():
    ...
    if os.path.exists(CSV) and os.path.getsize(CSV) > 0:
        ...
        if need.issubset(set(df.columns)):
            ...
            print(f"[plot] {CSV} present but missing columns {need - set(df.columns)}; will parse log")
    # fallback to log parse
    df = parse_log(LOG)
    if df.empty:
        raise SystemExit("No metrics found. Run a few CMWE commands with logging, e.g.\n"
...
if "answer_len" in df.columns and df["answer_len"].notna().any():
    ...
    print("Saved: logs/calibration_guard.png, logs/risk_vs_alpha.png, logs/risk_vs_len.png")
else:
    print("Saved: logs/calibration_guard.png, logs/risk_vs_alpha.png (answer_len not available)")
