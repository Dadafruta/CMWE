"""Generate citation varied big.

Run:
  python -m scripts.make_citation_varied_big --help
"""
...
def main():
    ...
    ap.add_argument("--risky", type=int, default=8000)
    args = ap.parse_args()
    rows = []
    for _ in range(args.risky):
        q = random.choice(RISKY_T).format(
            t=random.choice(TITLES),
            claim=random.choice(CLAIMS),
            topic=random.choice(TOPICS),
            nonsense=random.choice(NON),
