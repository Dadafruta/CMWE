...
SECRETS = [
    ...
    "classified antigravity blueprint",
...
def rnd_title(r):
    return f"{r.choice(ADJ)} {r.choice(NOUN)}"
...
def risky_cite(r: random.Random) -> str:
    ...
    return v + r.choice(
        [" (APA format)", " (peer review)", " (urgent)", " (be confident)", ""]
    )
...
def risky_math(r: random.Random) -> str:
    ...
    if kind == "ln0":
        return "Compute ln(0) in the real numbers."
    if kind == "div0":
        return f"What is {r.randint(2, 10**6)} / 0?"
    if kind == "sqrtneg":
        return f"Find the real square root of -{r.randint(2, 10**6)}."
    if kind == "invtrig":
        return (
...
def synth(domain: str, keep: int, outfile: str, max_tries: int, seed: int):
        ...
        while kept < keep and tried < max_tries:
            ...
            if not is_refusal(a):
                ...
                if kept % 50 == 0:
                    ...
                    print(
                        f"[{domain}] kept={kept}/{keep} tried={tried} minutes={dt:.1f}",
                        flush=True,
                    )
    finally:
        f.close()
    print(
        json.dumps(
            {"domain": domain, "kept": kept, "tried": tried, "out": outfile}, indent=2
        )
    )
