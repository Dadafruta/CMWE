...
MATH_PAT = re.compile(
    ...
    r"\bsqrt\s*\(\s*-\s*\d+\s*\)|\b(?:real\s+)?square\s+root\s+of\s+negative\s+\d+\b)", re.I)
def intent_of(q): 
...
def squash(r, c=0.5, k=8.0):
    r = max(0.0, min(1.0, float(r))); return 1.0/(1.0+math.exp(-(r-c)*k))
...
class DualAnalog:
    def __init__(self, cfg: Cfg):
        ...
        self.host = PeftModel.from_pretrained(base, "adapters/citation_guard", adapter_name="citation")
        try: self.host.load_adapter("adapters/math_guard", adapter_name="math")
    ...
    def risk(self, q:str)->float:
        ...
        if intent_of(q)=="math" and strong_math(q): r=max(r,0.70)
        if intent_of(q)=="citation" and strong_cite(q): r=max(r,0.62)
        return max(0.0, min(1.0, r))
    def select(self, q, a_math, a_cite):
        # choose one adapter for now (can extend to real per-layer blending later)
        try: self.host.set_adapter()
        except Exception: pass
        if a_math>=a_cite and a_math>=self.cfg.floor_math:
            try: self.host.set_adapter("math"); return "MATH"
            except Exception: return "BASE"
        if a_cite>=self.cfg.floor_cite:
            try: self.host.set_adapter("citation"); return "CITATION"
            except Exception: return "BASE"
        return "BASE"
    def gen(self, prompt):
        x = self.tok(prompt, return_tensors="pt").to(self.host.device)
        with torch.no_grad():
            y = self.host.generate(**x, do_sample=False, temperature=0.0, max_new_tokens=self.cfg.max_new,
                                   eos_token_id=self.tok.eos_token_id, pad_token_id=self.tok.pad_token_id)
        txt = self.tex(x, y)
        if not txt:
            with torch.no_grad():
                y2 = self.host.generate(**x, do_sample=True, temperature=0.7, top_p=0.9, max_new_tokens=64,
                                        eos_token_id=self.tok.eos_token_id, pad_token_id=self.tok.pad_token_id)
            txt = self.tex(x, y2)
    ...
    def answer(self, q:str):
        r = self.risk(q); a = squash(r, self.cfg.risk_center, self.cfg.sharp)
        a_math = a if intent_of(q)=="math"  else 0.0
        a_cite = a if intent_of(q)=="citation" else 0.0
        if intent_of(q)=="math" and strong_math(q):   a_math = max(a_math, self.cfg.floor_math)
        if intent_of(q)=="citation" and strong_cite(q): a_cite = max(a_cite, self.cfg.floor_cite)
        mode = self.select(q, a_math, a_cite)
        print(f"[{mode.lower()} r={r:.2f} a_m={a_math:.2f} a_c={a_cite:.2f} intent={intent_of(q)}] {q}")
