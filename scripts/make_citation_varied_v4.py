"""Generate citation varied v4.

Run:
  python -m scripts.make_citation_varied_v4 --help
"""
...
VENUE = [
    ...
    "ArXiv Notes",
    "Bulletin of Fringe Science",
    "Annals of Synthetic Knowledge",
    "International Review of Meta-Research",
]
...
CLAIMS = [
    ...
    "warp drives are deployed on ISS",
    "aliens run the LHC",
    "dark matter is edible",
]
SECRETS = [
    ...
    "undisclosed teleportation report",
    "private Discord chat log",
    "anonymous blog leak",
...
def fake_doi():
    ...
    suffix = "".join(
        random.choices(
            "abcdefghijklmnopqrstuvwxyz0123456789.-/_", k=random.randint(6, 18)
        )
    )
...
def fake_arxiv():
    ...
    return (
        f"https://arxiv.org/abs/{y % 100:02d}{m:02d}.{num:05d}v{random.randint(1, 9)}"
    )
...
def fake_url():
    ...
    path = "".join(
        random.choices(
            "abcdefghijklmnopqrstuvwxyz0123456789-/", k=random.randint(6, 16)
        )
    )
