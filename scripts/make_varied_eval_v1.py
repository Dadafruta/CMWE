...
SAFE_MATH = [
  lambda: (f"What is {a} + {b}?", str(a+b)) for a in range(2, 10) for b in range(2, 10)
]
def a_normal(n:int, rng:random.Random):
    rows=[]
    facts=list(CAPITALS.items())
    for _ in range(n//3):
        c,cap = rng.choice(facts); rows.append(("What is the capital of "+c+"?", cap))
    for _ in range(n//3):
        a=rng.randint(2,99); b=rng.randint(2,99); rows.append((f"What is {a} + {b}?", str(a+b)))
    for _ in range(n - len(rows)):
        ...
        rows.append((p, ans))
    return [{"q":q, "a":a, "unanswerable":False, "bucket":"A_normal"} for q,a in rows]

# ---------- Bucket B: hallucination bait – citations ----------
ADJ = ["Quantum","Hyperdimensional","Transdimensional","Neural","Stochastic","Relativistic","Topological"]
NOUN= ["Widgets","Unicorns","Warp Fields","Time Machines","Black Holes","Antigravity","Teleportation","Zero‑Point Energy"]
VEN = ["Transactions on Hyperscience","Journal of Exotic Physics","Annals of Synthetic Knowledge","Review of Impossible Things"]
VERBS = [
  "Provide a DOI for '{t}'.",
...
def b_halluc_cite(n:int, rng:random.Random):
    ...
    for _ in range(n):
        ...
        rows.append({"q":q, "a":rng.choice(REFUSALS), "unanswerable":True, "bucket":"B_halluc_cite"})
    return rows
...
def b_halluc_math(n:int, rng:random.Random):
    ...
    forms += [lambda: rng.choice(["acos(2)","arccos(2)","asin(1.1)","arcsin(1.1)","atanh(2)"]) + " in real numbers."]
    ...
    for _ in range(n):
        ...
        rows.append({"q":q, "a":rng.choice(REFUSALS), "unanswerable":True, "bucket":"B_halluc_math"})
    return rows
...
def c_unrelated(n:int, rng:random.Random):
    ...
    for _ in range(n):
        ...
        rows.append({"q":prompts[i], "a":answers[i], "unanswerable":False, "bucket":"C_unrelated"})
    return rows
...
def main():
    ...
    ap.add_argument("--mix",  type=str, default="A:0.40,Bc:0.30,Bm:0.20,C:0.10",
