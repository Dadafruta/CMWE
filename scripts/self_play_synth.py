...
def pick_refusal(domain):
    if domain == "cite":
        return random.choice(REFUSALS[:2])
    if domain == "math":
        return random.choice(REFUSALS[2:])
...
def math_risky_one():
    ...
    if kind == "ln0":
        ...
        forms = [
            f"What is {a}/0?",
            "Evaluate {a} ÷ 0.",
            "Give exact value of {a} over 0 (reals).",
        ]
    elif kind == "sqrtneg":
        n = -random.randint(2, 10**6)
        forms = [
            f"Find √({n}) (reals).",
            f"Compute sqrt({n}) over ℝ.",
            "Give real square root of {n}.",
        ]
    elif kind == "logneg":
        n = -random.randint(2, 10**6)
        b = random.choice(["2", "10", "e"])
        forms = [f"Compute log_{b}({n}) (reals)."]
    elif kind == "invtrig":
        forms = [
            random.choice(
...
def cite_risky_one():
    ...
    q = verb.format(
        t=random_title(), c=random.choice(CLAIMS), s=random.choice(SECRET)
    ) + random.choice(WRAPS)
    return q
...
def build(domain: str, n: int, outfile: str):
    ...
    while kept < n and tried < n * 30:
        ...
        if kept and kept % 200 == 0:
            print(
                f"[{domain}] kept {kept}/{n} (tried {tried}) elapsed {int(time.time() - t0)}s"
            )
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--n", type=int, default=6000, help="kept risky examples per domain"
    )
    args = ap.parse_args()
